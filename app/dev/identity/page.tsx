...
function TopBar() {
  return (
    <div className="flex items-center justify-between pt-5">
      <Link
        href="/dev/ui"
        className="inline-flex h-9 items-center gap-1 rounded-md px-2 text-[13px] font-medium text-ink-secondary hover:text-ink"
      >
...
function Eyebrow({ children }: { children: ReactNode }) {
  return (
    <p
      className="text-[12px] font-medium uppercase tracking-[0.14em] text-ink-tertiary"
    >
      {children}
    </p>
  );
  ...
  return (
    <span className="inline-flex items-baseline gap-1.5">
      <span
        ...
        style={{ backgroundColor: "var(--accent)" }}
      />
      <span className="text-[18px] font-bold uppercase tracking-[-0.02em] text-accent">
        Fokus
      </span>
    </span>
  );
    ...
    <div className="grid grid-cols-8 gap-2">
      {swatches.map((s) => (
        <div key={s.label} className="flex flex-col items-center gap-1.5">
          ...
          />
          <span className="text-[10px] font-medium text-ink">{s.label}</span>
      ...
      <section>
        ...
        <div className="mt-8 space-y-8">
          <TypeRow label="Display · 44px · 700 · tracking −0.02em">
            <h2
              className="text-[44px] font-bold tracking-[-0.02em] text-ink"
          ...
          <TypeRow label="Page title · 32px · 700">
            <h2
              className="text-[32px] font-bold tracking-[-0.02em] text-ink"
          ...
          <TypeRow label="Section title · 22px · 600">
            <h3 className="text-[22px] font-semibold leading-[1.2] text-ink">
              What you&apos;re really building
            </h3>
          </TypeRow>

          <TypeRow label="Headline · 17px · 600">
            <p className="text-[17px] font-semibold leading-[1.35] text-ink">
              The Why Chain
            </p>
          </TypeRow>
          ...
          <TypeRow label="Caption · 13px · 500 · ink-secondary">
            <p className="text-[13px] font-medium uppercase tracking-[0.08em] text-ink-secondary">
              Curiosity · 10 min
            </p>
          </TypeRow>
        </div>
...
function TypeRow({ label, children }: { label: string; children: ReactNode }) {
  return (
    <div>
      <p className="mb-2 text-[11px] font-medium uppercase tracking-[0.1em] text-ink-tertiary">
        {label}
      </p>
      {children}
    </div>
...
function MockChip({
  ...
  return (
    <span
      className={`inline-flex h-11 select-none items-center rounded-full px-5 text-[15px] font-medium ${
        selected
...
function MockPrimary({ children }: { children: ReactNode }) {
  return (
    <div
      className="flex h-14 w-full select-none items-center justify-center rounded-[14px] bg-accent text-[17px] font-semibold text-white"
...
function MockSecondary({ children }: { children: ReactNode }) {
  return (
    <div className="flex h-14 select-none items-center justify-center rounded-[14px] border border-line bg-bg-elevated px-5 text-[16px] font-medium text-ink">
      {children}
    </div>
  );
}
        ...
        <PhoneFrame>
          ...
          <div className="flex items-center justify-between">
            <button
              type="button"
              aria-label="Back"
              className="inline-flex h-9 items-center gap-1 rounded-md px-1 text-[15px] font-medium text-ink-secondary"
            >
      ...
      <section>
        ...
        <PhoneFrame tall>
          ...
          <div className="mt-10 flex items-center justify-between">
            <span className="text-[15px] font-medium text-ink-tertiary">
              Skip
            </span>
            <div
...
function IllustrationTile({
  ...
  return (
    <div className="flex flex-col items-center gap-3">
      ...
      <span className="text-[12px] font-medium uppercase tracking-[0.12em] text-ink-tertiary">
        {label}
      </span>
    </div>
  );
